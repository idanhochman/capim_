#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# CAPIM — EAGLE Alpaca trace collection (100 prompts, 8-bit).
#
# Usage from a notebook cell, AFTER cloning the repo (GPU runtime, >= sm_75 e.g.
# Kaggle T4):
#     !git clone https://github.com/idanhochman/capim_.git
#     !bash capim_/sim/scripts/kaggle_collect_eagle_alpaca.sh
#
# 8-bit (LLM.int8): base int8, EAGLE draft head stays FP16 (EaModel loads it
# separately from the base, so it is NOT quantized). Same 100 Alpaca prompts as
# the MEDUSA run -> paired comparison. Output: traces/vicuna7b_eagle_alpaca.json
# ---------------------------------------------------------------------------
set -euo pipefail
...
echo "==> EAGLE on Alpaca (100 prompts, 8-bit) ..."
python -u sim/scripts/collect_traces.py \
    --model-family vicuna7b --method eagle --dataset alpaca --n-prompts 100 --load-in-8bit

echo ""
echo "==> DONE. Saved: traces/vicuna7b_eagle_alpaca.json"
echo "    Sanity-check: log_probs VARY (not all -ln(vocab)); tree STRUCTURE varies"
echo "    step-to-step (dynamic); acceptance HIGHER than the Medusa Alpaca run."
